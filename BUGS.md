...
| B-004 | confirmed | Reproduced via Playwright (coordinator). `loadedSystems[system]=true` is set synchronously when a lazy load *starts*, before the real async parse finishes. Confirmed both halves: (1) switching to a procedure and clicking through its stages immediately can run stage logic before the just-requested system's registry entries actually exist — e.g. `Intervertebral_disc_L3_L4` was provably absent from `body3d.registry` while `lumbar-puncture`'s stage 1 was already rendering; degrades gracefully (a null focus point just skips the camera reframe, no crash, confirmed zero console errors), but is a real silent miss, not merely theoretical; (2) switching between two procedures that each need a different not-yet-loaded system overwrites the single global `window.onBody3DSystemLoaded`, silently dropping the first procedure's completion callback. Practical severity varies by procedure — landmarks that happen to sit close together (as in lumbar-puncture) mask the symptom, since the previous stage's camera framing looks approximately right anyway; procedures with more spatially separated landmarks would show it more visibly. | `Procedurtraning/js/procedures3d.js`, `Kroppsatlas/js/body3d.js` | 2026-07-25 @ `d72a273` | — |
| B-005 | confirmed | Reproduced via Playwright (coordinator) — turned out to be simpler and more deterministic than the original hypothesis, not really a "race": `resetProcedure3DStage()` clears overlay/selection but never touches clip state at all, so **every** reset leaves a previously-cut mesh (e.g. Skin) still clipped, 100% reproducible, not timing-dependent. Same for switching to a different procedure without resetting first — `loadProcedure3D()` has the identical gap. Confirmed both independently. Did not conclusively confirm the second half of the original hypothesis (an in-flight, still-growing animation actively mutating material state *after* a reset) — by the time a clean measurement window was set up, the animation had already finished growing; the missing-cleanup finding alone is sufficient to confirm the bug regardless. | `Kroppsatlas/js/body3d.js` (`_body3dAnimateIncision`, `resetProcedure3DStage`, `loadProcedure3D`) | 2026-07-25 @ `d72a273` | — |
