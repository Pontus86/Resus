...
| B-002 | suspected | Several `manifest.js` entries are duplicated (`Tibia.r`/`.l`, `Clavicle.r`/`.l`, `Patella.r`/`.l` each appear twice) — unclear if harmless (last one wins silently) or a symptom of something wrong in an early merge pass. | `Kroppsatlas/models/body/manifest.js` | 2026-07-25 @ `0585416` | — |
...
| ID | Status | Title | Where | Found | Related task |
|---|---|---|---|---|---|
| TODO-001 | open | Finish Procedurträning: cricothyrotomy, 5th of 5 planned procedures. Weakest real-anatomy backing of the set (laryngeal skeleton was never segmented in BodyParts3D) — needs a real design conversation, not just a mechanical data entry. | `Procedurtraning/` | 2026-07-25 @ `0585416` | — |
| TODO-002 | open | Combine the local clip-hole incision with lattice-style vertex deformation at the cut edges, for a more organic "pulled back" look instead of a hard-edged hole. Sandbox prototype exists (outside the repo); never integrated into the real module. | `Procedurtraning/js/procedures3d.js`, `Kroppsatlas/js/body3d.js` | 2026-07-25 @ `0585416` | — |
| TODO-003 | open | Wire up EKG's already-committed but unused 3D heart model (`EKG/models/heart/*.js` — aorta/LAD/LCx/RCA/LV/RV/LA/RA/pulmonary trunk) into the EKG module. Could visualize STEMI culprit-vessel territory in 3D alongside the existing 2D matching game. | `EKG/` | 2026-07-25 @ `0585416` | — |
| TODO-004 | open | Mine the rest of `Models/Body/Body` (raw BodyParts3D source library) for other useful anatomy not yet pulled into Kropps-atlas. | `Kroppsatlas/models/body/` | 2026-07-25 @ `0585416` | — |
