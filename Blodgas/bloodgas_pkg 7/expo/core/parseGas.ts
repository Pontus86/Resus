...
export function toAppInputs(parsed: ParsedGas): Record<string, number> {
  ...
  const copy = (from: string, to: string) => {
    if (v[from] !== undefined) out[to] = v[from];
  };
  ...
  if (v.stdBicarb !== undefined) out.HCO3 = v.stdBicarb;
  else if (v.actualBicarb !== undefined) out.HCO3 = v.actualBicarb;
  if (v.osmolality !== undefined) out.Osm = v.osmolality;
  return out;
}
