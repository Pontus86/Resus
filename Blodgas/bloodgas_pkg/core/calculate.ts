...
export interface BloodGasResults {
  ...
  naClGap?: number; // (Na - Cl) - 33, SWESEM Step 3 surrogate
  AGc?: number;
  AGalb?: number;
  urinAG?: number;
...
export function calculate(input: BloodGasInput): BloodGasResults {
  ...
  // --- Na–Cl gap (SWESEM Step 3, Stewart-derived bedside surrogate) ---
  // Δ(Na−Cl) = (Na − Cl) − 33. A normal Na−Cl difference is ~33; a HIGH value
  // (> ~3 above 0) points toward a metabolic ALKALOSIS (or a compensated
  // respiratory acidosis), a LOW value toward a NAGMA — independently of the
  // anion gap. Useful when albumin/phosphate aren't available for full Stewart.
  if (has(Na, Cl)) r.naClGap = (Na as number) - (Cl as number) - 33;
  ...
  if (has(pCO2)) r.deltapCO2 = 5.3 - (pCO2 as number);
