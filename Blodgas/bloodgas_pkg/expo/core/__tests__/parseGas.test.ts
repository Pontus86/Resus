...
// Regression: implausible OCR/LLM values must be rejected on import.
{
  const r = parseGas('pH -27\npCO2 3,8\ncHCO3-(P,st)c 12\ncNa+ 9000\ncK+ 4,2');
  const app = toAppInputs(r);
  const ok = app.pH === undefined && app.Na === undefined && app.HCO3 === 12 && app.K === 4.2;
  console.log(ok ? '  ok implausible values rejected on import' : '  FAIL plausibility: ' + JSON.stringify(app));
  if(!ok) process.exitCode = 1;
}
