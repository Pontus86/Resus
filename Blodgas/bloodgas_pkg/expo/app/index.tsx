            ...
            <>
              <View style={s.interpPanel}>
                ...
                {result.lines.map((l, i) => {
                  ...
                  const ng = result.calc?.naClGap;
                  const ngNote = ng === undefined ? '' : ng > 3 ? ' – talar för metabol alkalos' : ng < -3 ? ' – talar för NAGMA' : '';
                  return (
                    <React.Fragment key={i}>
                      ...
                      {isMetAcid && ng !== undefined && (
                        <View style={s.v4Row}>
                          <Text style={s.v4Th}>Na–Cl-gap</Text>
                          <View style={s.v4Td}><Text style={s.v4Val}>{fmt(ng, 1)}{ngNote}</Text></View>
                        </View>
                      )}
                    </React.Fragment>
                  );
...
const SHARED_REF: Record<string, [number, number]> = {
  ...
  Lac: [0, 2.3], Ca: [1.15, 1.33], Mg: [0.7, 0.95], Alb: [34, 45], PO4: [0.7, 1.6],
...
function buildResult(input: BloodGasInput) {
  ...
  d('Anjongap', r.anionGap, 'mmol/L', [6, 12]);
  d('Na–Cl-gap', r.naClGap, 'mmol/L', [-3, 3]);
  d('Δ anjongap', r.anionGapDelta, 'mmol/L', null);
  d('Delta-delta (ΔAG + HCO₃⁻)', r.extraImbalance, 'mmol/L', [22, 26]);
